...
async def list_skills(
    request: Request,
    source: Optional[str] = Query(None, description="Filter by source: builtin, workspace"),
) -> list[SkillInfo]:
    ...
    if source is None or source == "workspace":
        ...
        if workspace_skills_dir.exists():
            for skill_dir in sorted(workspace_skills_dir.iterdir()):
                if skill_dir.is_dir():
                    skill_info = _load_skill(skill_dir, "workspace")
                    if skill_info:
...
async def list_sources() -> list[dict[str, str]]:
    ...
    """
    return [
        {"id": "builtin", "name": "Built-in Skills"},
        {"id": "workspace", "name": "Custom Skills"},
    ]
