...
class WeixinChannel(BaseChannel):
    ...
    async def _download_media_item(
        ...
        try:
            ...
            if not filename:
                ts = int(time.time())
                hash_seed = encrypt_query_param or full_url or str(ts)
            ...
            if file_path.exists():
                ts = int(time.time())
                h = abs(hash(str(file_path) + str(ts))) % 100000
                file_path = media_dir / f"{file_path.stem}_{h}{file_path.suffix}"
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if not ctx_token:
            logger.warning(
                ...
                "This usually means no prior message was received from this user.",
                msg.chat_id,
            )
            return
