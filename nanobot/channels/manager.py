...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        if self.config.gateway.web_ui.enabled:
            logger.info("Web channel enabled (via WebConnectionManager)")
    ...
    async def _dispatch_outbound(self) -> None:
        ...
        while True:
            try:
                ...
                # Handle web channel specially via WebConnectionManager
                if msg.channel == "web":
                    try:
                        from nanobot.web.connection_manager import get_web_connection_manager
                        manager = get_web_connection_manager()
                        await manager.handle_outbound(msg)
                    except Exception as e:
